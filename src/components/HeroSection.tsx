  ...
  const slides = [
    {
      ...
      btnText: "Shop Now",
      bg: "https://foodinstitute.com/wp-content/uploads/2023/03/d6tu_l3chle.jpg.webp",
    },
    {
      id: 2,
      icon: (
  ...
  return (
    <div className="relative w-[98%] mx-auto mt-32 h-[80vh] rounded-3xl overflow-hidden shadow-2xl">
        <AnimatePresence mode="wait">
            ...
            >
                <Image src={slides[current]?.bg} alt="slide"
                priority
                fill
        ...
        <div className="absolute inset-0 flex items-center justify-center text-center text-white px-6">
            ...
            className="flex flex-col items-center justify-center gap-6 max-2-3xl"
            >
                <div className="bg-white/10 backdrop-blur-md p-6 rounded-full shadow-lg">{slides[current].icon}</div>
                <h1 className="text-3xl dm:text-5xl md:text-6xl font-extrabold tracking-tight drop-shadow-lg">{slides[current].title}</h1>
                <p className="text-lg sm:text-xl text-gray-200 max-w-2xl">{slides[current].subtitle}</p>
                <motion.button type="button" className="mt-4 bg-white text-green-700 hover:bg-green-100 px-8 py-3 rounded-full font-semibold shadow-lg transition-all duration-300 flex items-center gap-2"
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-3">
            {
                slides.map((_,idx)=>(
                    <button key={idx} className={`w-3 h-3 rounded-full transition-all ${
